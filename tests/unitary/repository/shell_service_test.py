from scipion_testrunner.repository import shell_service

__COMMAND = 'echo Hi'

def test_returns_expected_ok_return_code():
  assert (
    shell_service.run_shell_command(__COMMAND)[0] == 0
  ), "Command returned a non-zero exit status."

def test_returns_command_output():
  output = shell_service.run_shell_command(__COMMAND)[1]
  assert (
...
def test_returns_expected_error_return_code():
  assert (
    shell_service.run_shell_command("qwerty")[0] != 0
