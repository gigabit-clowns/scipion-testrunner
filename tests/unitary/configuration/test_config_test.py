...
def test_returns_empty_fields(_mock_log):
...
def test_returns_expected_fields(_mock_open, _mock_json_load):
...
def test_exits_on_file_not_found_error(
    _mock_open_raise_file_not_found_error, _mock_log
):
...
def test_exits_on_is_a_directory_error(
    _mock_open_raise_is_directory_error, _mock_log
):
    with pytest.raises(SystemExit):
        test_config.get_test_config(__DUMMY_FILE_PATH)
    _mock_log.assert_called_once_with(
...
def test_exits_on_permission_error(_mock_open_raise_permission_error, _mock_log):
    with pytest.raises(SystemExit):
        test_config.get_test_config(__DUMMY_FILE_PATH)
    _mock_log.assert_called_once_with(
...
def test_exits_on_json_decode_error(
    _mock_open, _mock_json_load_raise_json_decode_error, _mock_log
):
    with pytest.raises(SystemExit):
        test_config.get_test_config(__DUMMY_FILE_PATH)
    _mock_log.assert_called_once_with(
...
def test_exits_on_exception(_mock_open_raise_exception, _mock_log):
    with pytest.raises(SystemExit):
        test_config.get_test_config(__DUMMY_FILE_PATH)
    _mock_log.assert_called_once_with(
...
@pytest.fixture
def _mock_open_raise_file_not_found_error(_mock_open):
    _mock_open.side_effect = FileNotFoundError(__EXCEPTION_TEXT)


@pytest.fixture
def _mock_open_raise_is_directory_error(_mock_open):
    _mock_open.side_effect = IsADirectoryError(__EXCEPTION_TEXT)


@pytest.fixture
def _mock_open_raise_permission_error(_mock_open):
    _mock_open.side_effect = PermissionError(__EXCEPTION_TEXT)


@pytest.fixture
def _mock_json_load_raise_json_decode_error(_mock_json_load):
    _mock_json_load.side_effect = __JSON_EXCEPTION


@pytest.fixture
def _mock_open_raise_exception(_mock_open):
    _mock_open.side_effect = Exception(__EXCEPTION_TEXT)
