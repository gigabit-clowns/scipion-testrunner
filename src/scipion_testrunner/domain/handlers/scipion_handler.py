...
def get_all_tests(scipion: str, plugin_module: str):
    ...
    if not test_list and not python_handler.exists_python_module(plugin_module):
        logger.log_error(
            f"ERROR: No tests were found for module {plugin_module}. Are you sure this module is properly installed?"
        )
...
def download_datasets(scipion: str, datasets: List[str]):
    ...
    if failed_downloads:
        logger.log_error(
            "The download of at least one dataset ended with errors. Exiting."
        )
...
def run_tests(
    ...
    for batch_number in range(n_batches):
        ...
        failed_in_batch = __run_test_batch(
            test_batches[batch_number], max_jobs, scipion, plugin_module
        )
...
def __download_dataset(dataset: str, scipion: str) -> Optional[str]:
    ...
    ret_code, output = shell_handler.run_shell_command(
        f"{scipion} testdata --download {dataset}"
    )
    if ret_code:
        logger(
            logger.red(
...
def __run_test_batch(
    ...
    logger(
        logger.blue(
            f"Running a total of {batch_size} {test_number_text} for {plugin_module}{batch_text}..."
        )
    )
    return python_handler.run_function_in_parallel(
        __run_test, scipion, plugin_module, parallelizable_params=tests, jobs=jobs
    )
...
def __run_test(test: str, scipion: str, plugin_module: str) -> Optional[str]:
    ...
    ret_code, output = shell_handler.run_shell_command(
        f"{scipion} {__get_test_prefix(plugin_module)}{test}"
    )
    if ret_code:
