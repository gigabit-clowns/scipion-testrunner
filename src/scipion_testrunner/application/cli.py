...
def __add_params(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to Scipion executable, relative or absolute",
    )
    parser.add_argument(
        test_service.PLUGIN_PARAM_NAME, help="Name of the plugin's Python module"
    )
    parser.add_argument(
        "-j",
        f"--{test_service.JOBS_PARAM_NAME}",
        type=int,
        default=multiprocessing.cpu_count(),
        help="Number of jobs. Defaults to max available",
    )
    parser.add_argument(
        f"--{test_service.NO_GPU_PARAM_NAME}",
        action="store_true",
        help="If set, no tests that need a GPU will run. Use it in enviroments where a GPU cannot be accessed.",
    )
    parser.add_argument(
        f"--{test_service.TEST_DATA_PARAM_NAME}",
        default="",
        help="Location of the test data JSON file.",
    )
    return parser
...
def __get_args_from_parser(parser: argparse.ArgumentParser) -> Dict:
    ...
    if args[test_service.TEST_DATA_PARAM_NAME]:
        args[test_service.TEST_DATA_PARAM_NAME] = os.path.abspath(
            args[test_service.TEST_DATA_PARAM_NAME]
        )
    return args
